...
# 2. Loading of data
getwd()
data = read.csv("C:/Users/SAI/Desktop/FINANCE-24/FA1.csv")
head(data)
```
...
cols <- c("Company_Name","Company_year","X1","X2","X3","X4","X5","X6","X7","X8","X9","X10","X11","BSE","Target","X12","X13")
colnames(data) <- cols
head(data)
```
...
library(e1071)
model1 <- glm(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13,
...
model2 <- train(as.factor(Target) ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13,  
...
library(e1071)
library(caret)
svm_model <- svm(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13, data=training,kernel = "linear",probability = TRUE)
summary(svm_model)
```
...
svm_cross <- train(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13, data = training, method = "svmLinear",
trControl=trctrl,
...
library(e1071)
svm_model1 <- svm(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13, data=training,kernel = "radial",na.action = na.omit)
summary(svm_model1)
```
...
svm_cross1 <- train(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13, data = training, method = "svmRadial",
trControl=trctrl,
preProcess = c("center", "scale"),
tuneLength = 10)
...
randomf=randomForest(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13 , data =training)
randomf
```
...
set.seed(51)
# Training algorithm
rf_cross <- train(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13 , data = training,
...
```{r}
library(ipred)
bag <- bagging(Target~X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13, data = training,nbagg = 100,coob = TRUE,
...
print("Confusion Matrix for Bagging Before Cross-validation for TRAINING data")
...
set.seed(51)
# Training algorithm
bag_cross <- train(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13 , data = training,
...
```{r}
library(adabag)
m = boosting(Target~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13, data = training,boos=TRUE, mfinal=50)
```
...
# Training algorithm
m1 = boosting(Target~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13, data = training,boos=TRUE, mfinal=50,v=10)
```
...
#Build the model
model<-nnet(Target~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13, data = training,size = 4,decay = 0.0001,maxit = 500)
...
#Build the model
model_cross <- train(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13 , data = training,
...
mda_model= mda(Target~X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13, data = training )
mda_model
```
...
```{r}
# AUC-Score:
print("The AUC-score of MDA for Testing dataset before Cross validation ")
...
```{r}
#Build the model
mda_cross <- train(Target ~ X1+X2+X3+X4+X5+X6+X7+X8+X9+X10+X11+X12+X13 , data = training,
